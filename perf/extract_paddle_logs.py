...
parser.add_argument(
    "--log_dir", type=str, default="./logs/paddle-plsc/arcface", required=True)
parser.add_argument(
    "--output_dir", type=str, default="./result", required=False)
...
def extract_info_from_file(log_file, result_dict, speed_dict):
    ...
    batch_size_per_device = fname.split("_")[1]
    ...
    speed_dict[model][run_case][precision][batch_size_per_device][
        test_iter] = avg_speed
    average_speed = compute_average(speed_dict[model][run_case][precision][
        batch_size_per_device])
    median_speed = compute_median(speed_dict[model][run_case][precision][
        batch_size_per_device])

    result_dict[model][run_case][precision][batch_size_per_device][
...
def compute_speedup(result_dict, speed_dict):
    ...
    for m in model_list:
        run_case = [key for key in result_dict[m]
                    ]  # eg.['4n8g', '2n8g', '1n8g', '1n4g', '1n1g']
        for d in run_case:
            precision = [key for key in result_dict[m][d]]
            for p in precision:
                batch_size_per_device = [key for key in result_dict[m][d][p]]
                for b in batch_size_per_device:
                    speed_up = 1.0
                    if result_dict[m]['1n1g'][p][b]['median_speed']:
                        speed_up = result_dict[m][d][p][b][
                            'median_speed'] / result_dict[m]['1n1g'][p][b][
                                'median_speed']
                    result_dict[m][d][p][b]['speedup'] = round(speed_up, 2)
